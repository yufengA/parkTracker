...
set _DeploySetParametersFile=%RootPath%SEP_ASS3.SetParameters.xml
)
...
echo. "%MSDeployPath%\msdeploy.exe" -source:archiveDir='%RootPath%Archive' -dest:%_Destination% -verb:sync -disableLink:AppPoolExtension -disableLink:ContentExtension -disableLink:CertificateExtension -setParamFile:"%RootPath%SEP_ASS3.SetParameters.xml" %_MsDeployAdditionalFlags%
"%MSDeployPath%\msdeploy.exe" -source:archiveDir='%RootPath%Archive' -dest:%_Destination% -verb:sync -disableLink:AppPoolExtension -disableLink:ContentExtension -disableLink:CertificateExtension -setParamFile:"%RootPath%SEP_ASS3.SetParameters.xml" %_MsDeployAdditionalFlags%
)
goto :eof
...
:usage
echo =========================================================
if not exist "%RootPath%SEP_ASS3.deploy-readme.txt" (
...
start notepad "%RootPath%SEP_ASS3.deploy-readme.txt"
)
echo =========================================================
goto :eof
...
:CheckParameterFile
echo =========================================================
if  exist "%RootPath%SEP_ASS3.SetParameters.xml" (
echo SetParameters from:
echo "%RootPath%SEP_ASS3.SetParameters.xml"
echo You can change IIS Application Name, Physical path, connectionString
echo or other deploy parameters in the above file.
) else (
echo SetParamterFiles does not exist in package location.
